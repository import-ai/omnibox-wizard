...
class XProcessor(HTMLReaderBaseProcessor):
    ...
    async def convert(self, html: str, url: str) -> GeneratedContent:
        ...
        images: list[Image] = []
        images_dom: Tag | None = next(next(content.parent.parent.next_sibling.children).children)
        if images_dom:
            ...
            for image_tag in image_selection:
                if src := image_tag.get("src"):
                    images.append(Image.model_validate(
                        {"name": image_tag.get("alt", src), "link": src, "data": "", "mimetype": ""}
                    ))

        markdown: str = "\n\n".join([f"![{image.name or (i + 1)}]({image.link})" for i, image in enumerate(images)])
        if content:
